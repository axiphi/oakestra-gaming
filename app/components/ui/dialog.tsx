...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <BaseDialog.Popup
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-[90%] -translate-x-1/2 -translate-y-1/2 scale-[calc(1-0.1*var(--nested-dialogs))] gap-4 rounded-md border bg-popover p-4 text-popover-foreground shadow-md outline-hidden duration-200 data-ending-style:scale-95 data-starting-style:scale-95 data-ending-style:opacity-0 data-starting-style:opacity-0 sm:max-w-96",
        className,
      )}
      {...props}
    >
      ...
      <DialogClose
        data-dialog-close
        className="absolute right-4 top-4 rounded-xs text-muted-foreground opacity-50 transition-opacity hover:opacity-100 focus:outline-hidden"
      >
